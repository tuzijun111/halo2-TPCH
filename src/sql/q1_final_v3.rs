...
impl<F: Field> TestChip<F> {
    ...
    pub fn configure(meta: &mut ConstraintSystem<F>) -> TestCircuitConfig<F> {
        ...
        let is_zero_advice_column2 = [meta.advice_column(); 2];
        ...
        let config = LtEqGenericChip::configure(
            ...
            |meta| vec![meta.query_advice(lineitem[6], Rotation::prev())],
        );
                ...
                for i in 0..lineitem.len() {
                    for j in 0..lineitem[0].len() {
                        region.assign_advice(
                            ...
                            i,
                            || Value::known(lineitem.clone()[i][j]),
                        )?;
                    }
                ...
                for i in 0..l_combined.len() {
                    for j in 0..l_combined[0].len() {
                        region.assign_advice(
                            ...
                            i,
                            || Value::known(l_combined.clone()[i][j]),
                        )?;
                    }
                    if i > 0 {
                        ...
                        compare_chip[1].assign(
                            &mut region,
                            i,
                            &[l_combined.clone()[i - 1][4]],
                            &[l_combined.clone()[i][4]],
                        )?;

                        compare_chip[2].assign(
                            &mut region,
                            i,
                            &[l_combined.clone()[i - 1][5]],
                            &[l_combined.clone()[i][5]],
                        )?;

                        equal_v1_chip.assign(
                            &mut region,
                            i,
                            Value::known(l_combined.clone()[i][4] - l_combined.clone()[i - 1][4]),
                        )?;
                        equal_condition_chip.assign(
                            ...
                            (
                                Value::known(
                                    l_combined.clone()[i][4] - l_combined.clone()[i - 1][4],
                                ),
                                Value::known(
                                    l_combined.clone()[i][5] - l_combined.clone()[i - 1][5],
                                ),
                            ),
                        )?;
                    }
...
mod tests {
    ...
    fn test_1() {
        ...
        let right_shipdate = Fp::from(3730);
