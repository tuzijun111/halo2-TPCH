...
impl<F: Field + Ord> TestChip<F> {
    ...
    pub fn configure(meta: &mut ConstraintSystem<F>) -> TestCircuitConfig<F> {
        ...
        //     |meta| {
        //         let q_enable = meta.query_selector(q_enable[0]);
        //         let check = meta.query_advice(check[0], Rotation::cur());
            ...
            |mut region| {
                ...
                for i in 0..lineitem.len() {
                    ...
                    if lineitem[i][3] > condition[1] {
                        l_check.push(true);
                        // self.config.q_join[6].enable(&mut region, i)?; // used to select the valid rows for permutation
                    } else {
                        l_check.push(false);
                    }
                ...
                //                 join_config[j],
                //                 i,
                //                 || Value::known(join_value[k][i][j].clone()),
                //             )?;
                //         }
                //     }
